...
internal sealed class AffixManager : IDisposable
{
    ...
    private int _flagAliasExpected;
    ...
    private bool VariantMatchesCompoundPosition(string? variant, CompoundPosition position)
    {
        ...
        bool hasCompound = VariantHasFlag(variant, _compoundFlag);
        bool hasBegin = VariantHasFlag(variant, _compoundBegin);
        bool hasMiddle = VariantHasFlag(variant, _compoundMiddle);
        bool hasEnd = VariantHasFlag(variant, _compoundEnd);
        bool hasOnlyInCompound = VariantHasFlag(variant, _onlyInCompound);

        bool hasExplicitPosition = hasCompound || hasBegin || hasMiddle || hasEnd;
        bool onlyAsFallback = hasOnlyInCompound && !hasExplicitPosition;

        return position switch
        {
            CompoundPosition.Begin => hasBegin || hasCompound || onlyAsFallback,
            CompoundPosition.Middle => hasMiddle || hasCompound || onlyAsFallback,
            CompoundPosition.End => hasEnd || hasCompound || onlyAsFallback,
            _ => hasCompound || onlyAsFallback
        };
    }

    private bool VariantsContainFlag(IEnumerable<string> variants, string? token)
        => VariantsContainFlagWithAppend(variants, null, token);

    private bool VariantsContainFlagWithAppend(IEnumerable<string> variants, string? appendedFlags, string? token)
    {
        if (string.IsNullOrEmpty(token)) return true;

        foreach (var variant in variants)
        {
            var normalized = variant ?? string.Empty;
            if (_hashManager.VariantContainsFlagAfterAppend(normalized, appendedFlags, token))
            {
                return true;
            }
        }
    ...
    private void LoadAffix(string affixPath, string? preloadedContent = null)
    {
        ...
        if (_flagAliasTable.Count > 1)
        {
            _hashManager.SetFlagAliases(_flagAliasTable);
        }
    }
    ...
    private void ProcessAffixLine(string line)
    {
        ...
        switch (command)
        {
            ...
            case "AF":
                HandleFlagAliasDirective(line, parts);
                break;
    ...
    private void HandleFlagAliasDirective(string line, string[] parts)
    {
        if (_flagAliasTable.Count <= 1 && parts.Length == 2 && int.TryParse(parts[1], out var aliasCount))
        {
            _flagAliasExpected = aliasCount;
            _flagAliasTable.Clear();
            _flagAliasTable.Add(string.Empty);
            return;
        }

        if (_flagAliasExpected <= 0)
        {
            return;
        }

        var idx = line.IndexOf(' ');
        var aliasValue = idx >= 0 ? line.Substring(idx + 1).Trim() : string.Empty;
        if (string.IsNullOrEmpty(aliasValue) && parts.Length > 1)
        {
            aliasValue = string.Join(' ', parts.Skip(1));
        }
        ...
        _flagAliasTable.Add(aliasValue);

        if (_flagAliasTable.Count - 1 >= _flagAliasExpected)
        {
            _flagAliasExpected = 0;
        }
    }
    ...
    private void ParseAffixRule(string[] parts, bool isPrefix)
    {
        ...
        // Note: We must be careful not to skip a rule like "SFX Z a 0 ..." where
        // the affix is "0" (empty). We check if the 3rd token is Y/N (cross product).
        if (int.TryParse(parts[3], out _) && (parts[2].Equals("Y", StringComparison.OrdinalIgnoreCase) || parts[2].Equals("N", StringComparison.OrdinalIgnoreCase)))
        {
            return;
        }
    ...
    private bool TryMatchFlagOnce(string word, string pattern, int wordPos, int nextPatternPos, string flag, List<string> matchedParts)
    {
        ...
        for (int len = _compoundMin; len <= word.Length - wordPos; len++)
        {
            ...
            bool PartHasToken(string p, string token)
            {
                ...
                if (TryFindAffixBase(p, allowBaseOnlyInCompound: true, out var baseCandidate, out _, out var appendedFlag, out _, out _))
                {
    ...
    private bool CheckCompoundRecursive(string word, int wordCount, int position, string? previousPart, int syllableCount, bool requiresForceUCase)
    {
        ...
        if (position >= word.Length)
        {
            ...
            if (ok && wordCount >= 3 && LooksLikeMultiWordCompoundTypo(word))
            {
                return false;
            }
        ...
        for (int i = position + _compoundMin; i <= word.Length; i++)
        {
            ...
            var contribution = 1;
            if (IsCompoundMadeOfTwoWords(part, wordCount, position, i, word, out var aInnerForce, out var bInnerForce))
            {
                contribution = 2;
    ...
    private bool LooksLikeMultiWordCompoundTypo(string word)
    {
        ...
        var replacements = GetCompoundTypoReplacementChars();
        if (replacements.Count == 0)
        {
            return false;
        }

        var normalized = word.Normalize(System.Text.NormalizationForm.FormC);
        for (int i = 0; i < normalized.Length; i++)
        {
            var original = normalized[i];
            foreach (var replacement in replacements)
            {
                ...
                var candidate = string.Create(normalized.Length, (word: normalized, index: i, replacement), static (span, state) =>
                {
                    state.word.AsSpan().CopyTo(span);
    ...
    private List<char> GetCompoundTypoReplacementChars()
    {
        var tryChars = _options.TryGetValue("TRY", out var chars) && !string.IsNullOrEmpty(chars)
            ? chars
            : DefaultTryCharacters;

        if (_compoundTypoReplacementChars is not null && string.Equals(_cachedTrySequenceForCompoundTypos, tryChars, StringComparison.Ordinal))
        {
            return _compoundTypoReplacementChars;
        }

        var seen = new HashSet<char>();
        var list = new List<char>(tryChars.Length * 2);
        ...
        foreach (var ch in tryChars)
        {
            ...
            if (char.IsLetter(ch))
            {
                AddChar(char.ToLowerInvariant(ch));
                AddChar(char.ToUpperInvariant(ch));
            }
        }

        _cachedTrySequenceForCompoundTypos = tryChars;
        _compoundTypoReplacementChars = list;
        return list;
    }
    ...
    private bool IsValidCompoundPart(string part, int wordCount, int startPos, int endPos, string fullWord, out bool requiresForceUCase)
    {
        ...
        if (flags is null)
        {
            ...
            if (TryFindAffixBase(affixPart, allowBaseOnlyInCompound: true, out var tempBase, out var tempKind, out var tempAppended, out _, out _, requiredCompoundFlags))
            {
                ...
                if (affixBase is not null && EvaluateAffixBaseCandidate(part, affixBase, matchKind, appendedFlag, wordCount, startPos, endPos, fullWord, out requiresForceUCase))
                {
                    return true;
                }
            }
                ...
                if (!string.Equals(pattern.BeginChars, "0", StringComparison.Ordinal) && part.StartsWith(pattern.Replacement, StringComparison.Ordinal))
                {
                    ...
                    if (origBegin.Length >= _compoundMin)
                    {
                        ...
                        if (TryFindAffixBase(origBegin, true, out var replacementBase, out var replacementKind, out var replacementAppend, out _, out _, requiredCompoundFlags) &&
                            ...
                            EvaluateAffixBaseCandidate(part, replacementBase, replacementKind, replacementAppend, wordCount, startPos, endPos, fullWord, out requiresForceUCase))
                        {
                            return true;
                        }
                    }
                    ...
                    if (origEnd.Length >= _compoundMin)
                    {
                            ...
                            if (origFlags2 is not null && CandidateHasRequiredFlag(origEnd, null, pattern.EndFlag))
                        {
                            ...
                            if (TryFindAffixBase(origEnd, true, out var replacementBase2, out var replacementKind2, out var replacementAppend2, out _, out _, requiredCompoundFlags) &&
                                ...
                                EvaluateAffixBaseCandidate(part, replacementBase2, replacementKind2, replacementAppend2, wordCount, startPos, endPos, fullWord, out requiresForceUCase))
                        {
                            return true;
                        }
                    }
                        ...
                        if (string.Compare(fullWord, boundaryStart, pattern.Replacement, 0, pattern.Replacement.Length, StringComparison.Ordinal) == 0)
                        {
                            ...
                            if (augmentedPart.Length >= _compoundMin)
                            {
                                ...
                                if (TryFindAffixBase(augmentedPart, true, out var augmentedBase, out var augmentedKind, out var augmentedApp, out _, out _, requiredCompoundFlags) &&
                                    ...
                                    EvaluateAffixBaseCandidate(augmentedPart, augmentedBase, augmentedKind, augmentedApp, wordCount, startPos, endPos, fullWord, out requiresForceUCase))
                                {
                                    return true;
                                }
                            }
            ...
            if (flags is null)
            {
                if (IsCompoundMadeOfTwoWords(part, wordCount, startPos, endPos, fullWord, out var aInnerPart, out var bInnerPart))
                {
        ...
        if (wordCount == 0)
        {
            if (_compoundBegin is not null)
            {
                if (!variants.Any(v => !string.IsNullOrEmpty(v) &&
                                       VariantMatchesCompoundPosition(v ?? string.Empty, CompoundPosition.Begin) &&
                                       ...
                                       !(!string.IsNullOrEmpty(_forbiddenWordFlag) && _hashManager.VariantContainsFlagAfterAppend(v ?? string.Empty, null, _forbiddenWordFlag))))
                {
                    if (TryFindAffixBase(affixPart, true, out var affixBasePos, out var affixKindPos, out var affixAppendPos, out _, out _, requiredCompoundFlags) &&
                        EvaluateAffixBaseCandidate(part, affixBasePos, affixKindPos, affixAppendPos, wordCount, startPos, endPos, fullWord, out var affixForcePos))
                    {
            ...
            {
                if (!variants.Any(v => !string.IsNullOrEmpty(v) &&
                                       VariantMatchesCompoundPosition(v ?? string.Empty, CompoundPosition.Any) &&
                                       !(!string.IsNullOrEmpty(_compoundForbidFlag) && _hashManager.VariantContainsFlagAfterAppend(v ?? string.Empty, null, _compoundForbidFlag)) &&
                                       !(!string.IsNullOrEmpty(_forbiddenWordFlag) && _hashManager.VariantContainsFlagAfterAppend(v ?? string.Empty, null, _forbiddenWordFlag))))
                {
                    if (TryFindAffixBase(affixPart, true, out var affixBasePos, out var affixKindPos, out var affixAppendPos, out _, out _, requiredCompoundFlags) &&
        ...
        else if (endPos < fullWord.Length)
        {
            ...
            {
                if (!variants.Any(v => !string.IsNullOrEmpty(v) &&
                                       VariantMatchesCompoundPosition(v ?? string.Empty, CompoundPosition.Middle) &&
                                       !(!string.IsNullOrEmpty(_compoundForbidFlag) && _hashManager.VariantContainsFlagAfterAppend(v ?? string.Empty, null, _compoundForbidFlag)) &&
                                       !(!string.IsNullOrEmpty(_forbiddenWordFlag) && _hashManager.VariantContainsFlagAfterAppend(v ?? string.Empty, null, _forbiddenWordFlag))))
                {
                    if (TryFindAffixBase(affixPart, true, out var affixBasePos, out var affixKindPos, out var affixAppendPos, out _, out _, requiredCompoundFlags) &&
        ...
        else
        {
            ...
            {
                if (!variants.Any(v => !string.IsNullOrEmpty(v) &&
                                       VariantMatchesCompoundPosition(v ?? string.Empty, CompoundPosition.End) &&
                                       !(!string.IsNullOrEmpty(_compoundForbidFlag) && _hashManager.VariantContainsFlagAfterAppend(v ?? string.Empty, null, _compoundForbidFlag)) &&
                                       !(!string.IsNullOrEmpty(_forbiddenWordFlag) && _hashManager.VariantContainsFlagAfterAppend(v ?? string.Empty, null, _forbiddenWordFlag))))
                {
                    if (TryFindAffixBase(affixPart, true, out var affixBasePos, out var affixKindPos, out var affixAppendPos, out _, out _, requiredCompoundFlags) &&
        ...
        var requiredPosition = wordCount == 0
            ? CompoundPosition.Begin
            : endPos < fullWord.Length
                ? CompoundPosition.Middle
                : CompoundPosition.End;
        bool meetsRequiredCompoundFlags = requiredCompoundFlags.Count == 0 ||
                                          mergedVariants.Any(mv => !string.IsNullOrEmpty(mv) && VariantMatchesCompoundPosition(mv, requiredPosition));
    ...
    /// </summary>
    private bool EvaluateAffixBaseCandidate(string surfacePart, string? affixBase, AffixMatchKind matchKind, string? appendedFlag, int wordCount, int startPos, int endPos, string fullWord, out bool requiresForceUCase)
    {
        requiresForceUCase = false;
        ...
        if (_twoWordCheckDepth == 0 && affixBase is not null && IsCompoundMadeOfTwoWords(affixBase, wordCount, startPos, endPos, fullWord, out var aInnerAffix, out var bInnerAffix))
        {
        ...
        if (mergedVariants.Count > 0)
        {
            ...
            if (wordCount == 0)
            {
                if (mergedVariants.Any(m => !string.IsNullOrEmpty(m) &&
                                            VariantMatchesCompoundPosition(m, position) &&
    ...
    private bool CheckCompoundRulesWithReplacement(string word, int prevEnd, int currentEnd, string? previousPart, string currentPart, out bool replacementApplied)
    {
        ...
        if (_compoundPatterns.Count > 0 && previousPart is not null)
        {
            ...
            foreach (var pattern in _compoundPatterns)
            {
                ...
                var endCandidates = new List<(string Part, string? Appended, bool Reconstructed)> { (prevSurface, null, false) };
                for (int cut = Math.Max(0, prevSurface.Length - 1); cut >= 0; cut--)
                {
                    ...
                    // via affix rules (when used in compounds)
                        ...
                        {
                            if (!endCandidates.Any(e => e.Part == tail)) endCandidates.Add((tail, null, false));
                    }
                        else if (TryFindAffixBase(tail, true, out var tailBase, out _, out var tailApp, out _, out _))
                    {
                        if (tailBase is not null && tailBase.Length >= _compoundMin)
                        {
                                if (!endCandidates.Any(e => e.Part == tailBase && e.Appended == tailApp)) endCandidates.Add((tailBase, tailApp, true));
                        }

                            if (!endCandidates.Any(e => e.Part == tail && e.Appended == tailApp)) endCandidates.Add((tail, tailApp, false));
                    }
                }
                ...
                for (int cut = _compoundMin; cut <= currSurface.Length - _compoundMin; cut++)
                {
                    ...
                    {
                        if (!beginCandidates.Any(e => e.Part == head)) beginCandidates.Add((head, null, false));
                    }
                    else if (TryFindAffixBase(head, true, out var headBase, out _, out var headApp, out _, out _))
                    {
                        if (headBase is not null && headBase.Length >= _compoundMin)
                        {
                            if (!beginCandidates.Any(e => e.Part == headBase && e.Appended == headApp)) beginCandidates.Add((headBase, headApp, true));
                        }

                        if (!beginCandidates.Any(e => e.Part == head && e.Appended == headApp)) beginCandidates.Add((head, headApp, false));
                    }
                }
                ...
                foreach (var endCandidate in endCandidates)
                {
                    ...
                    {
                        if (CheckCompoundPatternMatch(endCandidate.Part, beginCandidate.Part, pattern, endCandidate.Appended, beginCandidate.Appended, endCandidate.Reconstructed, beginCandidate.Reconstructed))
                        {
                            ...
                            if (!string.IsNullOrEmpty(pattern.Replacement) && previousPart is not null)
                            {
                                try
                                {
                                    ...
                                    var currPart = currentPart ?? string.Empty;
                                    // Compute word indices and fragments
                                    int prevStart = prevEnd - prevPart.Length;
                                    ...
                                    var endChars = pattern.EndChars == "0" ? prevPart : pattern.EndChars;
                                    var beginChars = pattern.BeginChars == "0" ? currPart : pattern.BeginChars;

                                    var endCharsLocal = pattern.EndChars == "0" ? string.Empty : (pattern.EndChars ?? string.Empty);
                                    var beginCharsLocal = pattern.BeginChars == "0" ? string.Empty : (pattern.BeginChars ?? string.Empty);

                                    if (!string.IsNullOrEmpty(endCharsLocal) && !string.IsNullOrEmpty(beginCharsLocal))
                                    {
                                        var prevPrefix = prevPart.Substring(0, Math.Max(0, prevPart.Length - endCharsLocal.Length));
                                        var currSuffix = currPart.Substring(Math.Min(currPart.Length, beginCharsLocal.Length));
                                        ...
                                        var endTokLen = string.Equals(pattern.EndChars, "0", StringComparison.Ordinal) ? endCandidate.Part.Length : (pattern.EndChars?.Length ?? 0);
                                        var beginTokLen = string.Equals(pattern.BeginChars, "0", StringComparison.Ordinal) ? beginCandidate.Part.Length : (pattern.BeginChars?.Length ?? 0);
                                        ...
                                        var segmentStart = prevEnd - prevPart.Length;
                                        if (segmentStart < 0) segmentStart = 0;
                                        var segmentLength = prevPart.Length + currPart.Length;
    ...
    private bool CheckCompoundPatternMatch(string prevPart, string currentPart, CompoundPattern pattern, string? prevAppended = null, string? currAppended = null, bool prevReconstructed = false, bool currReconstructed = false)
    {
        ...
        if (string.Equals(pattern.EndChars, "0", StringComparison.Ordinal))
        {
            ...
            if (string.IsNullOrEmpty(prevPart) || prevReconstructed || _hashManager.GetWordFlags(prevPart) is null) return false;
        }
        ...
        if (string.Equals(pattern.BeginChars, "0", StringComparison.Ordinal))
        {
            if (string.IsNullOrEmpty(currentPart) || currReconstructed || _hashManager.GetWordFlags(currentPart) is null) return false;
        }
    ...
    public bool IsForbiddenWord(string word)
    {
        ...
        if (TryFindAffixBase(word, allowBaseOnlyInCompound: false, out var baseCandidate, out _, out var appended, out _, out _))
        {
    ...
    public bool CheckAffixedWord(string word, bool allowBaseOnlyInCompound = false)
    {
        ...
        return TryFindAffixBase(word, allowBaseOnlyInCompound, out _, out _, out _, out _, out _);
    }
    ...
    public bool IsAffixDerivedFormForbiddenByCompound(string word)
    {
        ...
        if (!TryFindAffixBase(word, allowBaseOnlyInCompound: false, out var baseCandidate, out var kind, out var appended, out _, out _)) return false;
        if (string.IsNullOrEmpty(baseCandidate)) return false;
    ...
    private bool TryFindAffixBase(string word, bool allowBaseOnlyInCompound, out string? baseCandidate, out AffixMatchKind kind, out string? appendedFlag, out int affixCount, out int cleanAffixCount, IReadOnlyList<string>? requiredCompoundFlags = null, int depth = 0, bool skipPartialCircumfix = false)
    {
        ...
        bool TrySatisfiesRequiredCompoundFlags(string candidateBase, string? appended)
        {
            ...
            var position = requiredCompoundFlags.Contains(_compoundBegin)
                ? CompoundPosition.Begin
                : requiredCompoundFlags.Contains(_compoundMiddle)
                    ? CompoundPosition.Middle
                    : requiredCompoundFlags.Contains(_compoundEnd)
                        ? CompoundPosition.End
                        : CompoundPosition.Any;
            return merged.Any(m => !string.IsNullOrEmpty(m) && VariantMatchesCompoundPosition(m, position));
        }
    ...
    private bool TryFindAffixBaseCore(string word, bool allowBaseOnlyInCompound, out string? baseCandidate, out AffixMatchKind kind, out string? appendedFlag, out int affixCount, out int cleanAffixCount, IReadOnlyList<string>? requiredCompoundFlags, int depth, bool skipPartialCircumfix)
    {
            ...
            foreach (var sfx in orderedSuffixes)
        {
            ...
            foreach (var s2 in orderedSuffixes)
            {
                ...
                if (DictionaryContains(reconstructedDouble))
                {
                    ...
                    if (!VariantsContainFlag(baseVariants, s2.Flag)) continue;
            ...
            foreach (var pfx in orderedPrefixes)
            {
                ...
                if (DictionaryContains(nestedCandidate))
                {
                    ...
                    else
                    {
                        ...
                        affixCount = 2;
                        cleanAffixCount = IsClean(sfx.AppendedFlag) + IsClean(pfx.AppendedFlag);
                        return true;
                    }
                }
                ...
                if (depth < 2 && !_hashManager.HasPhTarget(nestedCandidate) && TryFindAffixBase(nestedCandidate, allowBaseOnlyInCompound, out var nestedBase, out var nestedKind, out var nestedAppended, out var nestedAffixCount, out var nestedCleanAffixCount, requiredCompoundFlags, depth + 1, skipPartialCircumfix))
                {
                    ...
                    // Validate flags for the suffix (sfx)
                    var combinedAppended = ConcatFlags(nestedAppended, pfx.AppendedFlag);
                    if (!VariantsContainFlagWithAppend(baseVariants, combinedAppended, sfx.Flag)) continue;

                    baseCandidate = nestedBase;
                    kind = AffixMatchKind.SuffixThenPrefix;
                    appendedFlag = ConcatFlags(sfx.AppendedFlag, ConcatFlags(nestedAppended, pfx.AppendedFlag));
                    affixCount = Math.Min(2, nestedAffixCount + 2);
                    cleanAffixCount = nestedCleanAffixCount + IsClean(sfx.AppendedFlag) + IsClean(pfx.AppendedFlag);
                    return true;
                }
            }
        }
        ...
        foreach (var pfx in _prefixes)
        {
            ...
            if (_complexPrefixes)
            {
                foreach (var inner in _prefixes)
                {
                    if (string.IsNullOrEmpty(inner.Affix)) continue;

                    var innerAffixNorm = NormalizeApostrophes(inner.Affix ?? string.Empty);
                    var baseFromPrefixNorm = NormalizeApostrophes(baseCandidateFromPrefix);
                    if (!baseFromPrefixNorm.StartsWith(innerAffixNorm, StringComparison.OrdinalIgnoreCase)) continue;

                    var afterInner = baseCandidateFromPrefix.Substring(inner.Affix!.Length);
                    var reconstructedInner = afterInner;
                    if (!string.IsNullOrEmpty(inner.Stripping) && inner.Stripping != "0")
                    {
                        reconstructedInner = inner.Stripping + afterInner;
                    }

                    if (!string.IsNullOrEmpty(inner.Condition) && inner.Condition != ".")
                    {
                        try
                        {
                            if (!Regex.IsMatch(reconstructedInner, "^" + inner.Condition, RegexOptions.CultureInvariant)) continue;
                        }
                        catch (ArgumentException)
                        {
                            continue;
                    ...
                    if (DictionaryContains(reconstructedInner))
                    {
                        ...
                        if (!VariantsContainFlagWithAppend(innerVariants, inner.AppendedFlag, pfx.Flag))
                        {
                            continue;
                        }
                        if (!allowBaseOnlyInCompound && !string.IsNullOrEmpty(_onlyInCompound) && innerVariants.Count > 0 && innerVariants.All(v => !string.IsNullOrEmpty(v) && v.Contains(_onlyInCompound)))
                        {
                            // inner base not allowed in isolation, try other prefixes
                        }
                        else
                        {
            ...
            foreach (var sfx in orderedSuffixes)
            {
                ...
                if (DictionaryContains(baseCandidate2))
                {
                    ...
                    else
                    {
                        ...
                        affixCount = 2;
                        cleanAffixCount = IsClean(pfx.AppendedFlag) + IsClean(sfx.AppendedFlag);
                        return true;
                    }
                }
                ...
                if (depth < 2 && !_hashManager.HasPhTarget(baseCandidate2) && TryFindAffixBase(baseCandidate2, allowBaseOnlyInCompound, out var nestedBaseFromRem, out var nestedKindFromRem, out var nestedAppendedFromRem, out var nestedAffixCountFromRem, out var nestedCleanAffixCountFromRem, requiredCompoundFlags, depth + 1, skipPartialCircumfix))
                {
                    ...
                    affixCount = Math.Min(2, nestedAffixCountFromRem + 2);
                    cleanAffixCount = nestedCleanAffixCountFromRem + IsClean(pfx.AppendedFlag) + IsClean(sfx.AppendedFlag);
                    return true;
                }
            }
        }
    ...
    private bool SatisfiesNeedAffixRequirement(string? baseCandidate, int affixCount, string? appendedFlag, int cleanAffixCount)
    {
        if (_needAffixFlag is null || string.IsNullOrEmpty(appendedFlag))
        {
            return true;
        }

        if (!_hashManager.VariantContainsFlagAfterAppend(string.Empty, appendedFlag, _needAffixFlag))
        {
            return true;
        }

        // If the base dictionary entry already required an affix, the current
        // transformation satisfied that requirement (base flags propagate to the
        // derived form). Only enforce an "extra" affix requirement when the
        // NEEDAFFIX flag was introduced solely by the affix chain.
        if (!string.IsNullOrEmpty(baseCandidate))
        {
            var baseVariants = _hashManager.GetWordFlagVariants(baseCandidate).ToList();
            if (baseVariants.Count > 0 && baseVariants.Any(v => _hashManager.VariantContainsFlagAfterAppend(v ?? string.Empty, null, _needAffixFlag)))
            {
                return true;
            }
        }

        return cleanAffixCount >= 1;
    }
    ...
    private bool IsCompoundMadeOfTwoWords(string word, int wordCount, int startPos, int endPos, string fullWord, out bool aRequiresForce, out bool bRequiresForce)
    {
        ...
        try
        {
            ...
            for (int i = _compoundMin; i <= word.Length - _compoundMin; i++)
            {
                ...
                bool firstAvailable = _hashManager.Lookup(a) || TryFindAffixBase(a, allowBaseOnlyInCompound: true, out _, out _, out _, out _, out _);
                bool secondAvailable = _hashManager.Lookup(b) || TryFindAffixBase(b, allowBaseOnlyInCompound: true, out _, out _, out _, out _, out _);

                // both pieces must be present directly or derivable by affix rules
                if (firstAvailable && secondAvailable)
                {
                    ...
                    var splitPos = startPos + i;
                    if (!IsValidCompoundPart(a, wordCount, startPos, splitPos, fullWord, out var aForce)) continue;
                    if (!IsValidCompoundPart(b, wordCount + 1, splitPos, endPos, fullWord, out var bForce)) continue;

                    // validate the internal boundary's compound rules
                    if (CheckCompoundRules(fullWord, splitPos, endPos, a, b))
                    {
                        // matched and valid
